...
const ContactForm = ({ contact = {}, onSubmit }) => {
    ...
    const formatPhone = (value) => {
        ...
        const onlyMums = value.replace(/\D/g, '');

        if (onlyMums.length <= 10) {
            // Telefone fixo (8 digitos)
            return onlyMums
                .replace(/^(\d{2})(\d)/, '($1) $2')
                .replace(/(\d{4})(\d)/, '$1 - $2');
        } else {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const newErrors = {}
        if (!formData.nome) newErrors.nome = 'Nome & obrigatorio';
        if (!formData.email) newErrors.email = 'Email & obrigatorio';
        if (!formData.telefone) newErrors.telefone = 'Telefone & obrigatorio';
